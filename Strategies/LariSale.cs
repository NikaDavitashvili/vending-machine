...
{
    class LariSale : ISalesStrategy
    {
        public void ChangeBack(decimal balance)
        {
            Console.WriteLine("\n\nMachine has: 20 tetri, 50 tetri, 1 lari, 5 lari.");
            ...
            Console.WriteLine($"\n\tYour Change\n20 Tetri - {counter_20}\n50 Tetri - {counter_50}\n1 Lari - {counter_100}\n5 Lari - {counter_500}");
            Console.WriteLine($"Your Total change is - {sumOfChange} Tetri\n\nMoney left on Balance - {leftOnBalance} Tetri");
        }
        ...
        public void PrintByCurrency(List<ItemsToDrop> list)
        {
            foreach (var item in list)
            {
                Console.WriteLine($"{item.itemsName} - {item.Code} - {item.Price} Tetri - {item.ItemsCount}");
            }
        }
